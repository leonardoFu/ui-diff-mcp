...
/**
 * Create overlay image with highlighted regions
 */
export async function createOverlayImage(
  ...
  const outputPath = path.join(artifactsDir, `overlay_${Date.now()}.png`);
  
  // Start with the current image as base
  const baseImage = sharp(currentPath);
  ...
  // Create overlay composites for each region
  const composites = regions.map((region, index) => {
    const [x, y, w, h] = region.bbox;
    
    // Create a semi-transparent red rectangle
    const overlay = Buffer.from(
      Array(w * h * 4).fill(0).map((_, i) => {
        const channel = i % 4;
        if (channel === 0) return 255; // Red
        if (channel === 1) return 0;   // Green
        if (channel === 2) return 0;   // Blue
        if (channel === 3) return 128; // Alpha (semi-transparent)
        return 0;
      })
    );
